...
Tab selection props also live on the root: `selectedTabId` (controlled), `defaultSelectedTabId` (uncontrolled initial), and `onTabChange` (called on every tab click either way). See [Tabs](#tabs).
...
**`FlyoutTemplate.Footer`** renders a primary and secondary action right-aligned inside `EuiFlyoutFooter`, secondary first. If no action is present, the footer is omitted entirely — no default Cancel button is added. Only the first instance of each action is rendered; for `PrimaryActionMenu` specifically, the first instance that actually has panels wins. `PrimaryAction` and `PrimaryActionMenu` are mutually exclusive: declaring both logs a dev warning and the menu wins (since it is the more recently added one). An empty `panels` array on `PrimaryActionMenu` counts as absent, so a lone empty menu omits the footer without warning.
...
- `content` and item `renderItem` are not supported (`?: never`); both are rejected at the type level and stripped at runtime with a dev warning.
...
| `isLoading`, `isDisabled` | `boolean` | Match `PrimaryAction`. |
...
Footer action buttons (`PrimaryAction`, `SecondaryAction`) are not derived; their `data-test-subj` passes through to the button as given. `PrimaryActionMenu` is the same for the trigger button; its popover panel gets a derived subject: `${dataTestSubj}Panel`. The panel is portalled outside the flyout root, so a Scout locator scoped to the flyout cannot reach it by default — use the panel subject instead.
