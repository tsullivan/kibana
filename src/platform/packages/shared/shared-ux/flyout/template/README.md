...
block content.
- `collapsed` — renders the compact header from the outset. See
...
`@kbn/flyout-info-blocks`. Takes `title`, `children`, and optional `size` and
`color`.
...
These parts are the whole of what the header renders. Free-form content — your own markup, a component, bare text — is dropped.

## Scroll behavior

See [`src/header/README.md`](src/header/README.md) for behavior details, the `collapsed` prop, and implementation notes.
...
`onClick`, and optional `iconType`, `isLoading`, and `isDisabled`.
...
`id`, `hasChildBackground`, `onClose`, `size`, `minWidth`, `type`, `maxWidth`, `paddingSize`, `ownFocus`, `resizable`, `onResize`, `onActive`, `outsideClickCloses`, `focusTrapProps`, and `closeButtonProps` are forwarded to the underlying `EuiFlyout` unchanged. `hideCloseButton`, `closeButtonPosition`, and `side` are not exposed, so the close affordance stays consistent across flyouts.

### Sessions and the flyout menu

The template defaults to `session="start"`, which registers the flyout with EUI's flyout manager as the start of a new session. Use `session="inherit"` for a flyout opened from within another one, and `session="never"` for a standard, unmanaged flyout. Pass `historyKey` to group flyouts into one session history, which the menu bar's history control then navigates.

The template always runs the flyout menu in `auto` mode. A lone flyout typically shows EUI's standard close button instead; a flyout that is part of a session with history shows the menu bar.

`flyoutMenuProps` is passed through to the menu bar, with one addition: when the header `title` is a plain string it seeds the menu's `title`, so history entries are labelled without repeating yourself. Anything you set in `flyoutMenuProps` wins.
