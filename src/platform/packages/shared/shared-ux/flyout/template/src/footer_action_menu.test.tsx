...
describe('FlyoutTemplate.Footer.PrimaryActionMenu', () => {
  ...
  it('panels built dynamically with content warn once in development and do not render content', async () => {
    const warnSpy = jest.spyOn(console, 'warn').mockImplementation(noop);

    const panelsWithContent = [
      {
        id: 0,
        items: [{ name: 'Item', onClick: noop }],
        content: <div>forbidden content</div>,
      },
    ] as unknown as FlyoutFooterMenuPanel[];

    renderMenu({ panels: panelsWithContent });
    await user.click(screen.getByRole('button', { name: /take action/i }));

    await screen.findByRole('menuitem', { name: 'Item' });
    expect(screen.queryByText('forbidden content')).not.toBeInTheDocument();

    const contentWarns = warnSpy.mock.calls.filter((call) => String(call[0]).includes('content'));
    expect(contentWarns.length).toBeGreaterThanOrEqual(1);
